...
gulp.task('rename', function ()  {
    ...
    if (!newname) {
        console.log('Please write the new state-mapping name, like: "gulp rename --name mywidgetset" --author "Author Name"');
        process.exit();
    }
    ...
    if (fs.existsSync(__dirname + '/admin/state-mapping.png')) {
        fs.renameSync(__dirname + '/admin/state-mapping.png',              __dirname + '/admin/' + newname + '.png');
    }
    if (fs.existsSync(__dirname + '/widgets/state-mapping.html')) {
        fs.renameSync(__dirname + '/widgets/state-mapping.html',           __dirname + '/widgets/' + newname + '.html');
    }
    if (fs.existsSync(__dirname + '/widgets/state-mapping/js/state-mapping.js')) {
        fs.renameSync(__dirname + '/widgets/state-mapping/js/state-mapping.js', __dirname + '/widgets/state-mapping/js/' + newname + '.js');
    }
    if (fs.existsSync(__dirname + '/widgets/state-mapping')) {
        fs.renameSync(__dirname + '/widgets/state-mapping',                __dirname + '/widgets/' + newname);
    }
    var patterns = [
        {
            match: /state-mapping/g,
            replacement: newname
        },
        {
            match: /state-mapping/g,
        ...
        {
            match: /State-mapping/g,
            replacement: newname ? (newname[0].toUpperCase() + newname.substring(1)) : 'State-mapping'
        },
        {
            match: /shady2k/g,
            replacement: author
